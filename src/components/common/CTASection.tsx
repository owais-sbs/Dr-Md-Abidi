...
export function CTASection({
  ...
  return (
    <section className="relative overflow-hidden text-white" style={{ backgroundColor: '#6281D1' }}>
      ...
      <div className="absolute bottom-0 right-0 w-64 h-64 rounded-full bg-orange-400/25 blur-3xl translate-x-1/3 translate-y-1/3 pointer-events-none" />
      ...
      <div className="container-page relative py-16 sm:py-20">
        ...
        >
          ...
          <motion.p variants={fadeUp} className="mt-4 text-lg text-white/85 leading-relaxed">
            {description}
          </motion.p>
          <motion.div variants={fadeUp} className="mt-8 flex flex-col sm:flex-row items-center justify-center gap-4">
            <Link
              to={primaryHref}
              className="btn-orange inline-flex items-center gap-2 px-7 py-3.5 shadow-lift text-sm"
            >
              {primaryLabel}
            </Link>
            <a
