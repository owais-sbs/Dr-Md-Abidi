...
export function ContactCTA() {
  return (
    <section className="relative overflow-hidden text-white" style={{ backgroundColor: '#6281D1' }}>
      ...
      <div className="container-page relative py-12 sm:py-20">
        <motion.div variants={staggerContainer} initial="hidden" whileInView="visible" viewport={viewport} className="max-w-2xl">
          ...
          <motion.p variants={fadeUp} className="mt-4 text-base sm:text-lg text-white/85 leading-relaxed">
          ...
          <motion.div variants={fadeUp} className="mt-7 flex flex-col sm:flex-row gap-3">
            <Link to="/contact-us/" className="btn-orange inline-flex items-center justify-center gap-2 px-7 py-3.5 shadow-lift text-sm">
              Contact Us <ArrowRight className="w-4 h-4" />
            </Link>
            <a href={site.phoneHref} className="inline-flex items-center justify-center gap-2 border-2 border-white/40 hover:border-white text-white font-semibold px-7 py-3.5 rounded-full transition-all text-sm hover:bg-white/10">
